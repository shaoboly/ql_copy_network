...
class Tran_En2De():
    ...
    def get_test_examples(self, data_dir,fname=None):
        ...
        return self._create_examples(os.path.join(data_dir, 'en2de.test'))

    def get_dev_examples(self, data_dir,fname=None):
        if fname != None:
            print(fname)
            return self._create_examples(os.path.join(data_dir, fname))
        return self._create_examples(os.path.join(data_dir, 'en2de.dev'))

    def get_train_examples(self, data_dir,fname=None):
        if fname != None:
            print(fname)
            return self._create_examples(os.path.join(data_dir, fname))
        return self._create_examples(os.path.join(data_dir, 'en2de.train'))
    ...
    def _create_examples(file_path):
        ...
        for line in open(file_path, 'r', encoding='utf-8', errors='ignore'):
            try:
                article,title = line.strip().split('\t')
            except:
                print(line)
                continue
            examples.append(InputExample(guid='1', article=article, summary=title))
        return examples
    ...
    def convert_example_to_feature(example, tokenizer, config, lf_tokenizer=None):
        ...
        # if config.vocab_file_out != None:
        #     lf_tokenizer = tokenization.FrTokenizer(vocab_file=config.vocab_file_out,
        ...
        # summary
        summary_tokens = lf_tokenizer.tokenize(example.true_summary)
        ...
        summary_ids = lf_tokenizer.convert_tokens_to_ids(summary_tokens)
