...
class Batcher(object):
    ...
    def __init__(self, processor, hps):
        ...
        self.BATCH_QUEUE_MAX = 1000 if self.is_train else 500000  # max number of batches the batch_queue can hold
    ...
    def prepare_examples(self):
        ...
        self.samples_number = len(self.examples)
        self.processor.log_statistics(self.examples)
    ...
    def sample_generator(self):
        ...
        lf_tokenizer=None
        if self._config.vocab_file_out!=None:
            if self._config.task_name == "en2de":
                lf_tokenizer = tokenization.FrTokenizer(vocab_file=self._config.vocab_file_out, do_lower_case=self._config.do_lower_case)
            else:
                lf_tokenizer = tokenization.FullTokenizer(vocab_file=self._config.vocab_file_out, do_lower_case=self._config.do_lower_case)
        dataset = self.examples
        while True:
            idx = np.random.permutation(len(dataset)) if self.is_train else range(len(dataset))
            for i in idx:
                if not self.is_train:
                    yield self.processor.convert_example_to_feature(dataset[i], tokenizer, self._config, lf_tokenizer)
                else:
                    feature = refine_train_summary(dataset[i], self._config)
                    yield self.processor.convert_example_to_feature(feature, tokenizer, self._config,lf_tokenizer)
            if not self.is_train:
                break

    def batch_from_sentence(self,sentence,tokenizer):
        ...
        example =InputExample(guid="1",article=sentence, summary="")

        feature = self.processor.convert_example_to_feature(example, tokenizer, self._config, None)

        samples = [Sample(feature,self._config,self._config.mode)]

        return Batch(samples,len(samples),self._config,self.mode)
    ...
    def __init__(self, hps):
        ...
        if hps.vocab_file_out != None:
            if hps.task_name == "en2de":
                lf_tokenizer = tokenization.FrTokenizer(vocab_file=hps.vocab_file_out,
                                                        do_lower_case=hps.do_lower_case)
            else:
                lf_tokenizer = tokenization.FullTokenizer(vocab_file=hps.vocab_file_out,
                                                          do_lower_case=hps.do_lower_case)

        self.features = [Sample(processor.convert_example_to_feature(example, tokenizer, hps,lf_tokenizer), hps, hps.mode)
