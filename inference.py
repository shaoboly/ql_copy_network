...
from utils.infer_data_processer import prepare_inf_features, prepare_inf_2_features
from utils.infer_utils import decode_target_ids, all_batch_already_decoded, find_first_pad_token,decode_target_ids_batch_beam,decode_target_ids_beam
from utils.metric_utils import write_batch_for_rouge, rouge_eval, rouge_log,write_batch_beam_result
...
def create_infer_op_2(dev_model):
    with dev_model.graph.as_default():
        return dev_model.decode_infer_2()
...
def single_stage_model_inference(dev_model, infer_op, batch_data, params):
    ...
    # all_beam_result = decode_seq[:, :, :]
    decode_seq = decode_seq[:, 0, :].tolist()
    return [decode_seq]

def single_stage_model_inference_topk(dev_model, infer_op, batch_data, params):
    """Inference for single model"""
    if hasattr(dev_model, 'sentence_rep'):  # fake feed data
        batch_data.tiled_sentence_rep = np.zeros((1, params.hidden_size))
    # get output of encoder
    ...
    decode_seq = run(dev_model, infer_op, new_batch_data)
    # [batch, top_beam, len] => [batch, len], [batch, top_beam] => [batch]
    all_beam_result = decode_seq[:, :params.top_beams, :]
    decode_seq = decode_seq[:, 0, :].tolist()
    return [decode_seq],all_beam_result


def two_stage_model_inference(dev_model, infer_op, infer2_op, batch_data, params):
    """Inference for two step model"""
    decode_seq = single_stage_model_inference(dev_model, infer_op, batch_data, params)[0]
    ...
    return [decode_seq, fine_tuned_seq]
...
def three_stage_model_inference(dev_model, infer_op, infer2_op, infer_sent_op, batch_data, params):
    ...
    fine_tuned_logits = run_infer_2(dev_model, infer2_op, new_batch_data)
    fine_tuned_seq = fine_tuned_logits.tolist()
    return [decode_seq, sent_fine_tuned_seq, fine_tuned_seq]


def inference(hps):
    # Prepare dir
    result_dir = os.path.join(hps.output_dir, hps.test_file + '-results/')
    ...
    while True:
        ...
        if infer_type == InferType.single:
            ids_results,decode_seq = single_stage_model_inference_topk(dev_model, pred_seq, batch, hps)
            #ids_results, all_result = single_stage_model_inference_topk(dev_model, pred_seq, batch, hps)
            res_dirs = [decode_dir]
            #write_batch_beam_result(batch, all_result, hps)
        ...
        idx = write_batch_for_rouge(batch_summaries, decode_result, idx, ref_dir, res_dirs, trunc_dec_dir,
                                    hps, batch.true_num)
        print(idx)
        write_batch_beam_result(batch,decode_seq, hps)
