...
class BertS2LCopySep(BaseModel):
    """
    ...
    def __init__(self, bert_config, batcher, hps):
        super(BertS2LCopySep, self).__init__(hps, bert_config, batcher)
    ...
    def _add_placeholders(self):
        ...
        # next decoder
        self.output_label_next = tf.placeholder(tf.int32, [None, None], name='output_label_next')  # [b, l_t], output_ids_oo
        self.output_mask_next = tf.sequence_mask(self.output_len-1,
                                            maxlen=tf.shape(self.output_label)[1],
                                            dtype=tf.float32)  # [b, l_t]
    ...
    def _build_summarization_model(self):
        ...
        with tf.variable_scope('decode'):
            ...
            def next_position_test():
                with tf.variable_scope('next_decoder'):
                    self.decoder_indicate_pos = tf.get_variable(
                        "output_weights", [hidden_size,out_dict_size+self.max_out_oovs],
                        initializer=tf.truncated_normal_initializer(stddev=0.02))
                    self.decoder_indicate_pos = tf.get_variable(
                        "output_bias", [out_dict_size+self.max_out_oovs],
                        initializer=tf.truncated_normal_initializer(stddev=0.02))



                    with tf.variable_scope('loss'):
                        self.ce_next = smooth_cross_entropy(
                            self.logits,
                            self.output_label,
                            self.hps.label_smoothing)

                        self.ce_next = tf.reshape(self.ce, tf.shape(self.output_label))  # [b, l_t]

                        self.loss_next = tf.reduce_sum(self.ce * self.output_mask) / tf.reduce_sum(self.output_mask)  # scalar
    ...
    def _make_input_key(self):
        ...
        self.output_keys_dev = ['loss', 'logits','pred_ids']
