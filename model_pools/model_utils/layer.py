...
def gate_res_layer(previous_data,
                input_data,
                   hidden_state,
                dropout_rate=None):
    previous_data = linear(previous_data,hidden_state)
    if dropout_rate is not None and dropout_rate > 0.0:
        input_data = tf.nn.dropout(input_data, 1 - dropout_rate)
...
def attention_bias(inputs, mode, inf=-1e9, name=None):
    ...
    with tf.name_scope(name, default_name="attention_bias", values=[inputs]):
        if mode == "causal":
            ...
            return m
        elif mode == "cloze_bias":
            length = inputs
            lower_triangle = tf.matrix_band_part(
                tf.ones([length, length]), -1, 0
            )
            ret = inf * (1.0 - lower_triangle)
            m = tf.matrix_band_part(ret, 0, 1)
            return tf.reshape(m, [1, 1, length, length])
        elif mode == "no_bias":
            length = inputs
            m = tf.zeros([length, length])
            return tf.reshape(m, [1, 1, length, length])
        elif mode == "distance":
            ...
            return tf.reshape(ret, [1, 1, length, length])
        elif mode == "mask_draft":
            length = inputs
            with tf.control_dependencies([tf.assert_equal(length % 2, 0)]):
                half = length / 2
                sub_matrix = tf.ones([half, half])
                upper_left = tf.matrix_band_part(sub_matrix, -1, 0)
                lower_left = tf.zeros([half, half])
                upper_right = 1-tf.matrix_band_part(sub_matrix, -1, 0)
                lower_right = tf.reverse(tf.reverse(tf.matrix_band_part(sub_matrix, -1, 0), [-1]), [-2])
                upper = tf.concat([upper_left, upper_right], -1)
                lower = tf.concat([lower_left, lower_right], -1)
                full = tf.concat([upper, lower], -2)
                #full = (1 - full) * inf
                full = tf.expand_dims(full, 0)
                full = tf.expand_dims(full, 0)
                return full
        elif mode == "mask_draft_warmup":
            length = inputs
            with tf.control_dependencies([tf.assert_equal(length % 2, 0)]):
                half = length / 2
                sub_matrix = tf.ones([half, half])
                upper_left = tf.matrix_band_part(sub_matrix, -1, 0)
                lower_left = tf.zeros([half, half])
                upper_right = tf.zeros([half, half])
                lower_right = tf.zeros([half, half])
                upper = tf.concat([upper_left, upper_right], -1)
                lower = tf.concat([lower_left, lower_right], -1)
                full = tf.concat([upper, lower], -2)
                #full = (1 - full) * inf
