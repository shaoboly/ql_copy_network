...
class BaseModel:
    def __init__(self, hps, bert_config, batcher):
        ...
        #self.gSess_train = tf.Session(config=tf.ConfigProto(allow_soft_placement=True), graph=self.graph)
    ...
    def create_or_load_recent_model(self):
        with self.graph.as_default():
            ...
            if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
                ...
                # self.save_model(self.hps.output_dir)
                # now_vars = tf.train.list_variables(self.hps.pretrain_dir)
                # init_vars = tf.train.list_variables(self.hps.pretrain_dir)
                # out_f = open("./my_test.txt","w")
                # for x in now_vars:
                #     out_f.write("{}\t{}\n".format(x[0],x[1]))
                # out_f.write("\n\n\n")
                # for x in init_vars:
                #     out_f.write("{}\t{}\n".format(x[0],x[1]))
                # exit(0)
                self._load_pretrain_s2s_assgin()
