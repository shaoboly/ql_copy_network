...
from model_pools.modeling import embedding_lookup, embedding_postprocessor, create_attention_mask_from_input_mask
from utils.copy_utils import calculate_final_logits, tf_trunct


# noinspection PyAttributeOutsideInit
class BertSummarizerDecV4V2(BaseModel):
    """
    Based on BertSummarizerDecV4V1.
    Using first n-1 golden token & last L - n draft token when predict n-th word in second decoder.
    """

    def __init__(self, bert_config, batcher, hps):
        super(BertSummarizerDecV4V2, self).__init__(hps, bert_config, batcher)
    ...
    def _add_placeholders(self):
        ...
        # should we feed draft summary or ground-truth
        self.feed_draft = tf.placeholder(tf.bool, [], name='feed_draft')
    ...
    def _build_summarization_model(self):
        ...
        dec_model = modeling.BertModel(
            ...
            input_mask=tf.squeeze(self.second_dec_attn_bias, 1),  # [b, 2 * l_t, 2 * l_t]
        ...
        self.masked_out_embed = self.out_embed * tf.expand_dims(change_ids_mask, -1)
        self.decoder_input = tf.pad(self.masked_out_embed, [[0, 0], [1, 0], [0, 0]])[:, :-1, :]  # Shift left
        # ################################################### decoding train - 2
        ...
        with tf.variable_scope('loss'):
            ...
            mle_2 = tf.reduce_sum(self.second_ce * output_mask, -1) / (tf.reduce_sum(output_mask, -1))  # [b]

            self.second_loss = tf.reduce_mean(tf.reduce_sum(self.second_ce * output_mask, -1) / (tf.reduce_sum(
                output_mask, -1)))  # scalar
    ...
    def _make_input_key(self):
        ...
        self.input_keys = ['source_ids', 'source_ids_oo', 'source_len', 'source_seg_ids', 'max_oov_num', 'target_ids',
                           'target_len', 'sent_level_attn_bias', 'target_ids_oo', 'reward', 'feed_draft']
