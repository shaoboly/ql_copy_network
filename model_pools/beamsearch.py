...
from model_pools.model_utils.common import *
...
def _beam_search_step(time, func, state, batch_size, beam_size, alpha, eos_id):
    ...
    flags = tf.equal(symbol_indices, eos_id)  # [b, 2 * beam]
    ...
    shape = candidate_seqs.get_shape()
    tri_flags = tf.py_func(filter_exist_tri_grams, [candidate_seqs], tf.bool, stateful=False)
    tri_flags.set_shape((shape[0], shape[1]))
    alive_scores = alive_scores + tf.to_float(tri_flags) * tf.float32.min  # [b, 2 * beam]
...
def create_inference_ops_general(model_infer_fn, params, feature_prefix):
    ...
    max_length = tf.fill([batch_size, beam_size], tf.constant(params.max_out_seq_length, tf.int32))  # [b, beam]
