...
class MultiGPUBaselineCopyAddOnlyDecoder(BaseModelMulti):
    """
    Based on BertSummarizerCopy, change some model settings, 41.9 on CNN/DM
    """

    def __init__(self, bert_config, batcher, hps):
        super(MultiGPUBaselineCopyAddOnlyDecoder, self).__init__(hps, bert_config, batcher)
        self.gSess_train = tf.Session(config=tf.ConfigProto(),
                                      graph=self.graph)
    ...
    def _build_summarization_model(self):
        ...
        for i, (input_ids, input_len, segment_ids, output_ids, output_len,
            ...
            with tf.device(assign_to_gpu(i, "/gpu:0")), tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
                ...
                #mask encoder_output
                encoder_output = tf.zeros_like(encoder_output)
