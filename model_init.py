...
class SummarizationModel(object):
    ...
    def __init__(self, hps, vocab,batcher):
        ...
        if self._hps.cell_name == "lstm":
            self.cell_build = LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)

        if self._hps.cell_name == "gru":
            self.cell_build = GRUCell(hps.hidden_dim)
    ...
    def _add_decoder(self, inputs):
        ...
        if hps.mode == "decode":
            ...
            outputs, out_state, attn_dists, p_gens, coverage = attention_decoder_fixed_context(inputs,
                                                                                               self._dec_in_state,
                                                                                               self._enc_states,
