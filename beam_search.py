...
class Hypothesis(object):
    ...
    def __init__(self, tokens, log_probs, state, attn_dists, p_gens, coverage, len):
...
def extand_hyp_to_batchsize(hyps):
    while(len(hyps)<FLAGS.batch_size):
        hyps.append(hyps[-1])
    pass

def run_beam_search(sess, model, vocab, batch, top_k):
