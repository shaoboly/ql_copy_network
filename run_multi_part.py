...
def reset_batcher_for_next_train(hps):
    task_name = hps.task_name.lower()
    if task_name not in processors:
        raise ValueError('Task not found: %s' % task_name)


    prefex = "EEMv507_6_2018-08-21.txt.train"

    previous_train_file = hps.train_file
    index = int(previous_train_file.replace(prefex,""))+1
    index = index%hps.part_num

    refname = "wikipedia_split_%04d" % index

    hps.train_file="{}{}".format(prefex,index)
    print("prepare for the next part: {}\n\n".format("{}{}".format(prefex,str(index))))
...
def train(hps):
    ...
    epoch_train_number = int(bert_model.batcher.samples_number / (hps.train_batch_size * hps.accumulate_step))
    for step in range(start_step, bert_model.num_train_steps):
        ...
        epoch_train_number-=1
        if epoch_train_number==0:
            train_batcher = reset_batcher_for_next_train(hps)
            bert_model.batcher=train_batcher
            epoch_train_number = int(bert_model.batcher.samples_number / (hps.train_batch_size * hps.accumulate_step))
