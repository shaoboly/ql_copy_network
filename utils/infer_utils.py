...
def decode_target_ids_batch_beam(decode_ids_list, batch, hps):
    #vocab_words = hps.vocab_words
    vocab_words = hps.vocab_words_out
    extra_vocab_list = batch.source_oovs
    decoded = []
    for decode_ids_beam, extra_vocab in zip(decode_ids_list, extra_vocab_list):
        all_beam = []
        extended_vocab = vocab_words + extra_vocab
        for decode_ids in decode_ids_beam:
            syms = []
            for idx in decode_ids:
                if isinstance(idx, six.integer_types) or isinstance(idx, np.int32):
                    sym = extended_vocab[idx]
                else:
                    sym = idx
                if sym == hps.pad:
            ...
            all_beam.append(syms)
        decoded.append(all_beam)
    return decoded

def decode_target_ids_beam(decode_ids_list, batch, hps):
    #vocab_words = hps.vocab_words
    vocab_words = hps.vocab_words_out
    extra_vocab_list = batch.source_oovs
    decoded = []
